...
export function createMockPuppeteer() {
	const page = {
		setExtraHTTPHeaders: vi.fn() as Mock<(headers: Record<string, string>) => void>,
		setViewport: vi.fn() as Mock<(viewport: { width: number; height: number }) => void>,
		...
		waitForSelector: vi.fn().mockResolvedValue(undefined) as Mock<
			(selector: string, options?: Parameters<Page["waitForSelector"]>[1]) => Promise<void>
		>,
		...
		emulateMediaType: vi.fn().mockResolvedValue(undefined) as Mock<
			(type: Parameters<Page["emulateMediaType"]>[0]) => Promise<void>
		>,

		$: vi.fn().mockResolvedValue(true) as Mock<(selector: string) => Promise<unknown>>,
	...
	const browser = {
		newPage: vi.fn().mockResolvedValue(page as unknown as Page) as Mock<() => Promise<Page>>,
